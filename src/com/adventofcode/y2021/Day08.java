...
public class Day08 {
    ...
    public static void main(String[] args) {
        ...
        long nbOfEasyDigits = Arrays
                .stream(lines)
                .flatMap(s -> Arrays.stream(s.substring(s.indexOf("|")).trim().split(" ")))
                .filter(s -> easyDigitsLength.contains(s.length()))
                .count();
        ...
        for (String line : lines){
            ...
            List<String> digits = Arrays.asList(lineSplit[1].split(" "));
            Map<String, Integer> mapDigits = findNumberForEachInput(inputs, digits);
            StringBuilder code = new StringBuilder();
            for (String digit : digits){
                code.append(mapDigits.get(orderString(digit)));
            }
    ...
    private static Map<String, Integer> findNumberForEachInput(List<String> inputs, List<String> digits) {
        Map<String, Integer> segmentNbOfOccurences = new HashMap<>();
        Stream.of(inputs.stream().collect(Collectors.joining()).split("")).forEach(s -> {
            if(segmentNbOfOccurences.containsKey(s)){
                segmentNbOfOccurences.put(s, segmentNbOfOccurences.get(s) + 1);
            } else {
                segmentNbOfOccurences.put(s, 1);
            }
        });
        ...
        //Easy digits
        //1
        mapNumbers.put(1, orderString(inputs.stream().filter(s -> s.length() == 2).findFirst().get()));
        mapDigits.put(mapNumbers.get(1), 1);
        //4
        mapNumbers.put(4, orderString(inputs.stream().filter(s -> s.length() == 4).findFirst().get()));
        mapDigits.put(mapNumbers.get(4), 4);
        //7
        mapNumbers.put(7, orderString(inputs.stream().filter(s -> s.length() == 3).findFirst().get()));
        mapDigits.put(mapNumbers.get(7), 7);
        //8
        ...
        String b = segmentNbOfOccurences.
                keySet().
                stream().
                filter(s -> segmentNbOfOccurences.get(s) == 6)
                .findFirst().get();

        String c = segmentNbOfOccurences.
                keySet().
                stream().
                filter(s -> segmentNbOfOccurences.get(s) == 8 && !s.equals(a))
                .findFirst().get();

        String d =
                //|_| _ | = |_
                //  |   |
                mapNumbers.get(4).replaceAll("[" + mapNumbers.get(1) + "]", "")
                //|_ - b = _
                .replace(b, "");

        String e = segmentNbOfOccurences.
                keySet().
                stream().
                filter(s -> segmentNbOfOccurences.get(s) == 4)
                .findFirst().get();

        String f = segmentNbOfOccurences.
                keySet().
                stream().
                filter(s -> segmentNbOfOccurences.get(s) == 9)
                .findFirst().get();

        String g = segmentNbOfOccurences.
                keySet().
                stream().
        ...
        //3
        mapDigits.put(orderString(a + c + d + f + g), 3);

        //5
        mapDigits.put(orderString(a + b + d + f + g), 5);

        //6
        mapDigits.put(orderString(a + b + d + e + f + g), 6);

        //9
        mapDigits.put(orderString(a + b + c + d + f + g), 9);
