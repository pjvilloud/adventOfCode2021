...
public class Day07 {
    ...
    public static final String SAMPLE_INPUT = "16,1,2,0,4,2,7,1,2,14";
    ...
    public static void main(String[] args) {
        List<Integer> positions = Arrays.stream(INPUT.split(",")).mapToInt(Integer::parseInt).boxed().collect(Collectors.toList());
        Map<Integer, Integer> fuelConsumptions = new HashMap<>();
        //Part 1
        for(int i = positions.stream().min(Integer::compareTo).get() ; i <= positions.stream().max(Integer::compareTo).get(); i++){
            int finalI = i;
            fuelConsumptions.put(i, positions.stream().mapToInt(pos -> Math.abs(pos - finalI)).sum());
        }
        System.out.println("They must spend " + fuelConsumptions.values().stream().mapToInt(Integer::intValue).min().getAsInt() + " fuel to align to position");

        //Part 2
        fuelConsumptions = new HashMap<>();
        for(int i = positions.stream().min(Integer::compareTo).get() ; i <= positions.stream().max(Integer::compareTo).get(); i++){
            int finalI = i;
            fuelConsumptions.put(i, positions.stream().mapToInt(pos -> sumOfInt(finalI, pos)).sum());
        }
        System.out.println("They must spend " + fuelConsumptions.values().stream().mapToInt(Integer::intValue).min().getAsInt() + " fuel to align to position");

    }

    private static int sumOfInt(int x, int y){
        return IntStream.range(0, Math.max(x, y) - Math.min(x, y) + 1).sum();
    }
